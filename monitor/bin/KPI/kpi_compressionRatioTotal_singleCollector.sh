...
# --------- Incoming raw record file size for an hour
  ...
  incomingRawFileSize='';incomingRawFileSize=`$SSH $cnp0vip "/bin/find $COLBKPDIR -mmin -300 -type f -name TrafficEventLog*${TZDATE}*.gz 2>/dev/null | xargs stat -c %s 2>/dev/null" | awk 'BEGIN { SUM=0 } { SUM += $1 } END { print SUM }'`
